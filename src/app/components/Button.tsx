...
export default function Button({
    ...
    switch (variant) {
        ...
        default:
            return (
                <button
                    className={`px-5 py-3 rounded-xl border-2 transition-all ease-in-out duration-300 ${disabled ? "bg-stone-400 border-stone-400" : "bg-orange-400 border-orange-400 hover:bg-orange-500 hover:border-orange-500"} ${className}`}
                    type={type}
                    disabled={disabled}
                    onClick={onClick}
                >
                    <Typography 
                        className={`font-medium ${disabled ? "text-stone-200" : "text-white"}`}
                        variant={"p"} 
                        text={text}
                    />
                </button>
